...
# Anomalias do clima
df_weather_anomally = spark.read.parquet(f"s3a://{raw_bucket}/anomalies/weather")
df_weather_anomally.printSchema()
df_weather_anomally = df_weather_anomally.join(df_cities, on="city", how="left")

# Quebra o UF-cidade em duas colunas separadas
df_weather_anomally = df_weather_anomally.withColumn("uf", upper(split("city", "-")[0])) \
                                         .withColumn("city", split("city", "-")[1])\
                                         .dropDuplicates()
df_weather_anomally.write.mode("overwrite").parquet(f"s3a://{refined_bucket}/weather_anomalies_with_coords")
...
df_airquality_anomally = spark.read.parquet(f"s3a://{raw_bucket}/anomalies/airquality")
df_airquality_anomally.printSchema()
df_airquality_anomally = df_airquality_anomally.join(df_cities, on="city", how="left").drop('hour')
df_airquality_anomally.show()

df_airquality_anomally = df_airquality_anomally.replace([np.nan, float('inf'), float('-inf')], None)
df_airquality_anomally['std_value'] = df_airquality_anomally['std_value'].replace([np.nan, float('inf'), float('-inf')], 0)
df_airquality_anomally['aqi_z_score'] = df_airquality_anomally['std_value'].replace([np.nan, float('inf'), float('-inf')], 0)

df_airquality_anomally = df_airquality_anomally.withColumn("uf", upper(split("city", "-")[0])) \
                                               .withColumn("city", split("city", "-")[1])\
                                               .dropDuplicates()
df_airquality_anomally.write.mode("overwrite").parquet(f"s3a://{refined_bucket}/airquality_anomalies_with_coords")
